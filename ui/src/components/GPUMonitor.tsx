...
const GpuMonitor: React.FC = () => {
  ...
  const content = useMemo(() => {
    ...
    if (!gpuData.hasGpuTool) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <strong className="font-bold">GPU が検出されませんでした</strong>
          <span className="block sm:inline"> nvidia-smi / xpu-smi がこのシステムで利用できません。</span>
    ...
    if (gpuData.gpus.length === 0) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">GPU monitoring tool ({gpuData.gpuBackend}) is available, but no GPUs found.</span>
        </div>
      );
    }
